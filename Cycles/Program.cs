...
namespace Cycles
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            string strNumberOfHours = Console.ReadLine();

            if (sbyte.TryParse(strNumberOfHours, out sbyte numberOfHours) && numberOfHours > 0 && numberOfHours <= 24)
            {
                Console.WriteLine("Please enter hourly rate");
                string strHourlyRate = Console.ReadLine();
                if (decimal.TryParse(strHourlyRate, out decimal hourlyRate) && hourlyRate > 0)
                {
                    Console.WriteLine($"Your payment per day: {numberOfHours * hourlyRate}");
                }
                else
                {
                    Console.WriteLine("Error ******************************************* Error");
            ...
            //7. Генерація таблиці множення для конкретного числа: Напишіть програму,
            //яка запитує в користувача число, а потім генерує таблицю
            //множення для цього числа від 1 до 10.

            sbyte multiplicationTableMaxFactor = 10;
            int firstFactor = 0; //ініціалізація нашого множника

            Console.WriteLine("Please enter you number");
            try
            {
                firstFactor = int.Parse(Console.ReadLine());
            }
            catch (Exception ex)
            {
            ...
            for (int i = 1; i <= multiplicationTableMaxFactor; i++)
            {
                Console.WriteLine($"{firstFactor} * {i} = {firstFactor * i}"); //вивід
            }

            //8. Перевірка на простоту: Напишіть програму, яка перевіряє,
            //чи є задане користувачем число простим. Число вважається
            //простим, якщо воно ділиться лише на 1 і на себе.

            Console.WriteLine("Please enter you number");
            int checkedNumberForPrime = 0;
            bool isPrime_2 = true;
            try
            {
                checkedNumberForPrime = int.Parse(Console.ReadLine());
            }
            catch (Exception ex)
            {
            ...
            if (checkedNumberForPrime == 2) isPrime_2 = true; //окрема перевірка на число 2
            //відкидуємо числа менше та рівні 1 та числа кратні двом, не включаючи двійку
            else if (checkedNumberForPrime <= 1 || checkedNumberForPrime % 2 == 0) isPrime_2 = false;
            else
            {   //перевіряємо непарні числа до корня з нашого числа
                for (int i = 3; i * i <= checkedNumberForPrime; i += 2)
                {
                    if (checkedNumberForPrime % i == 0)
                    {
                        isPrime_2 = false;
                        break;
                    }
            ...
            string resultIsPrime = isPrime_2 ? "Number is prime" : "Number is not prime";
            Console.WriteLine(resultIsPrime);
